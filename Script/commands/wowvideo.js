...
async function fastStream(links) {
  ...
  const streams = attempts.map(url =>
    axios({ method: "GET", url, responseType: "stream", headers: HEADERS, timeout: 18000, maxRedirects: 5 })
      .then(r => { r.data.path = "wowvideo.mp4"; return r.data; })
  );
  return Promise.any(streams);
}

module.exports.config = {
  name: "wowvideo", version: "3.0.0", hasPermssion: 0,
  credits: "BELAL BOTX666", description: "SAD VIDEO V2 ULTRA FAST",
  commandCategory: "video", usages: "wowvideo", cooldowns: 5
};
